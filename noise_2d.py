...
def power_spectrum(array):
    ...
    # Idea: fractals are scale-invariant
    ...
    # Fit a line in log-log space (ignore r=0 to avoid log(0))
    ...
    slope, intercept, r_value, p_value, std_err = linregress(log_r[5:-10], log_p[5:-10])
    ...
    return np.clip(D, 2.0, 3.0), log_r, log_p, slope, intercept
...
def variogram(array, num_samples=5000):
    ...
    bins = np.logspace(np.log10(1), np.log10(min(w, h)/2), num=50)
...
# def power_spectrum_diagnose(array):
#     """
#     Estimates fractal dimension using the Power Spectrum (Fourier) method.
#     """
#     # Compute 2D Fast Fourier Transform
...
#     # RETURN the arrays alongside D and the fit line parameters
#     return np.clip(D, 2.0, 3.0), log_r, log_p, slope, intercept

# def variogram_diagnose(array, max_dist=50, num_samples=5000):
    """
    Estimates fractal dimension using the Variogram (Hurst Exponent) method.
    Uses random sampling for performance on large arrays.
    """
    ...
    # Bin the distances to compute average variance per distance bin
    bins = np.logspace(np.log10(1), np.log10(max_dist), num=20)
    bin_means, bin_edges, _ = scipy.stats.binned_statistic(distances, variances, statistic='mean', bins=bins)
    bin_centers = (bin_edges[:-1] + bin_edges[1:]) / 2
    ...
    return np.clip(D, 2.0, 3.0), log_dist, log_var, slope, intercept